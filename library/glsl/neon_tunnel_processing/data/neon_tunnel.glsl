...
void main() {
    ...
    float rings = sin(depth * 7.0 - time * ringSpeed);
    float rails = sin(angle * 10.0 + time * railSpeed);
    float glow = pow(max(rings, 0.0), 12.0) + pow(max(rails, 0.0), 18.0) * 0.35;
    vec3 color = vec3(0.08, 0.01, 0.20) + glow * vec3(0.15, 0.85, 1.0);
    color += pow(max(sin(depth * 3.5 - time * (1.0 + pulse)), 0.0), 20.0) * vec3(1.0, 0.05, 0.55);
