...
- Editor pop-out toggle: `[pop]` in the action bar spawns the selected visual's source into its own WezTerm window (`wezterm cli spawn --new-window`) and closes the left pane; `[dock]` splits it back left (`split-pane --left`) and kills the pop-out window.
