...
- Headless preview mode: when a visual's route is preview-only (`preview` on, `window` off), `run_visual` sets `XVC_HEADLESS=1` in the launcher `.bat` and spawns with `CREATE_NO_WINDOW` (no OS window, no console). Each of the 7 Processing/GLSL sketches hides its surface via `if (System.getenv("XVC_HEADLESS") != null) surface.setVisible(false);` and keeps rendering `data/frame.png`. The 4 Nannou visuals do the same via `window.set_visible(false)` and emit `data/frame.png` every ~250 ms through nannou's async `window.capture_frame()` (their `frameout()` uses `current_dir()/data`, matches the launcher's cwd).
