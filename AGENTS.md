...
- Headless preview mode: when a Processing/GLSL visual's route is preview-only (`preview` on, `window` off), `run_visual` sets `XVC_HEADLESS=1` in the launcher `.bat` and spawns with `CREATE_NO_WINDOW` (no OS window, no console). Each of the 7 sketches hides its surface via `if (System.getenv("XVC_HEADLESS") != null) surface.setVisible(false);` and keeps rendering `data/frame.png`. Nannou visuals have no headless mode — their preview pane shows the "no frame" banner.
